 ...
 * be **wrong**. Lauren sweeps ±0.587m and Sam ±0.555m, so treating each as a cylinder demands
 * 1.142m between the marks — and the shipped placement is 1.090m apart. A test asserting the bound
 * would fail on a scene that has a measured 0.408m of clearance, because the bound assumes both of
...
describe('duet placement', () => {
  ...
  it('clears the meshes at the tightest moment of both loops', () => {
    // 0.408m bone-to-bone, of which roughly 0.16m is mesh rather than air. Re-run
...
describe('duet framing', () => {
  ...
  it('frames above each dancer’s own peak, at their own depth', () => {
     ...
     *
     * Measured at each dancer's mark rather than at the target. Sam's 2.047m jump is 0.91m further
