 ...
 *   Sam     Gangnam_Groove   ±0.541m lateral   ±0.553m deep   head peaks at 2.064m  10.21s
 *
 ...
 *
 * Sam's 2.064m is the clip jumping, and it is the single number that sets the top of the frame.
 ...
 *
 * Treated as two cylinders, clearing those radii side by side needs 0.587 + 0.541 = 1.128m between
 ...
 *
 *   lateral 0.50m   0.334m      lateral 0.70m   0.444m
 *   lateral 0.60m   0.398m      lateral 0.80m   0.494m
 *
 * Mesh thickness is roughly 0.08m of skin around a bone on each of them, so about 0.16m of that
 * figure is not air. 0.60m leaves 0.398m flat-on, about 0.24m of genuine gap at the very worst
 ...
 * improves from 0.398m to 0.427m. The clearance figures above are the flat-on ones, so the shipped
...
export const DUET_CLIP_EXTENTS: Record<string, ClipExtent> = {
  ...
  Gangnam_Groove: { deep: 0.553, lateral: 0.541, peak: 2.064 }
};
 ...
 */
export const DUET_MEASURED_CLEARANCE = 0.427;
 ...
 *
 * Level, at 0.9m, and 3.65m back: the position at which the pair's two vertical extremes meet the
 * frame together. The bottom one is Lauren's feet, the nearer figure. The top one is Sam's
 * 2.064m jump, which is 0.91m further back and so subtends less than its height suggests. Solving
 * `h / d = (2.064 − h) / (d + 0.91)` gives `h ≈ 0.9` at any distance near this. At 3.65m the swept
 * pair fills about 93% of the frame's height (`yarn duet:measure` prints the exact figure). The
 ...
 */
export const DUET_CAMERA = { fov: 32, position: [0, 0.9, 3.65] as const };
 ...
 * uses 93% of it. Vertical fit is therefore not a constraint at all. The visible world *width* is that
 * height times the aspect, so width is the only thing a container shape can take away.
 *
 *   below 0.74   crops a hand at the moment both dancers are at full lateral extension
 *   0.74         the pair exactly fits the frame's width
 *   0.80         the pair fills the frame both ways, the shape the stage is drawn at
 ...
 * floor from 0.83 to 0.74 and the natural shape from 0.92 to 0.80.
 *
...
export const DUET_NATURAL_ASPECT = 0.8;

/** The floor. Below this the sides crop; see the list above. */
export const DUET_MIN_ASPECT = 0.742;
